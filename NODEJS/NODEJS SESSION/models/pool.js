...
module.exports = {
  createUser: (email, password, callback) => {
    // Utilisez bcrypt.hash() pour hacher le mot de passe
    bcrypt.hash(password, saltRounds, (err, hash) => {
      if (err) {
        console.error("Erreur de hachage du mot de passe : " + err);
        return callback(err, null);
      } else {
        // Stockez le mot de passe haché (hash) dans la base de données
        connection.query(
  ...
  getUserByEmail: (email, callback) => {
    // Récupérez le mot de passe haché à partir de la base de données
